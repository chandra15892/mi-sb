...
//            client.blobName(name).buildClient().delete();
	...
	public List<String> blobListByContainer() {
		List<String> blobList = new ArrayList<>();

		try {

			PagedIterable<BlobItem> pa = blobContainerClient.listBlobs();
			if (pa != null) {
				Iterator<BlobItem> itr = pa.iterator();
				while (itr.hasNext()) {
					BlobItem blobItem = itr.next();
					blobList.add("Blob name: " + blobItem.getName() + ", Snapshot: " + blobItem.getSnapshot());
				}
				return blobList;
			}
